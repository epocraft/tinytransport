...
class Project
{
    ...
    /**
     * @var Collection<int, ProjectModule>
     */
    #[ORM\OneToMany(targetEntity: ProjectModule::class, mappedBy: 'project')]
    private Collection $projectProjectModules;
    ...
    public function __construct()
    {
        ...
        $this->projectProjectModules = new ArrayCollection();
     ...
     */
    public function getProjectProjectModules(): Collection
    {
        return $this->projectProjectModules;
    }

    public function addProjectProjectModule(ProjectModule $projectProjectModule): static
    {
        if (!$this->projectProjectModules->contains($projectProjectModule)) {
            $this->projectProjectModules->add($projectProjectModule);
            $projectProjectModule->setProject($this);
        }

        return $this;
    }

    public function removeProjectProjectModule(ProjectModule $projectProjectModule): static
    {
        if ($this->projectProjectModules->removeElement($projectProjectModule)) {
            // set the owning side to null (unless already changed)
            if ($projectProjectModule->getProject() === $this) {
                $projectProjectModule->setProject(null);
            }
        }
